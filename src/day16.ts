...
function decodePacket(padding:boolean) {
    const packetVersion = bin2dec(readNBits(3));
    const typeID =  bin2dec(readNBits(3));
    console.log(`Packet version: ${packetVersion}, type ID: ${typeID}`);
    logRemaining();
    ...
    if (typeID == 4) {
        ...
        console.log(`Number value: ${numberLiteral}`);
        ...
        console.log(`Lenght Type ID: ${lengthTypeID}`);
        if (lengthTypeID == '0') {
...
/*     console.log('------------------------------------------------------------')
    ...
    console.log('processed:',binInput.slice(0,pointer).join(''));
    console.log('remaining:',binInput.slice(pointer).join(''));
    console.log('------------------------------------------------------------')
 */}
